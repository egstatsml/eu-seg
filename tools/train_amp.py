...
def initialise_var_params(net):
    if 'bisenetv2' in cfg.model_type:
        ...
        bias_squared_sum = torch.zeros(
            net.module.head.conv_out.bias.shape).cuda()
    elif 'bisenetv1' in cfg.model_type:
        weight_squared_sum = torch.zeros(
            net.module.conv_out.conv_out.weight.shape).cuda()
        bias_squared_sum = torch.zeros(
            net.module.conv_out.conv_out.bias.shape).cuda()
        ...
        # is enet
        weight_squared_sum = torch.zeros(
            net.module.fullconv.weight.shape).cuda()
...
def update_var_params(net, weight_mean, bias_mean, weight_squared_sum,
    ...
    if 'bisenetv2' in cfg.model_type:
        ...
        # now update our sum parameters for the  weight and bias terms
        weight_squared_sum += torch.square(
            net.module.conv_out.conv_out.weight - weight_mean)
        ...
        weight_squared_sum += torch.square(
            net.module.final_layer.conv2.weight - weight_mean)
...
def train():
    ...
    weight_squared_sum, bias_squared_sum, step_count = initialise_var_params(
        net)
    ...
    for it, (im, lb) in enumerate(dl):
        ...
        if (it >= cfg.warmup_iters) and ('bayes' in cfg.model_config) and (
            ...
            weight_squared_sum, bias_squared_sum, step_count = update_var_params(
                net, w_mean, b_mean, weight_squared_sum, bias_squared_sum,
                step_count)
    ...
    if 'bayes' in cfg.model_config:
        ...
        torch.save(bias_var,
                   osp.join(cfg.respth, f'{cfg.model_type}_bias_var.pt'))
        print('weight_var = ', weight_var)
        print('bias_var = ', bias_var)
        bayes_state = add_mean_var_to_state(cfg.model_type,
                                            net.module.state_dict(), w_mean,
                                            b_mean, weight_var, bias_var)
        # now save the bayes state
        bayes_save_pth = osp.join(
            cfg.respth,
