...
parse.add_argument(
    ...
    dest='im_anns',
    type=str,
    default='./datasets/cityscapes/train.txt',
)
...
lb_missing_vals = [
    ind + min_lb_val for ind, el in enumerate(lb_hist.tolist()) if el == 0
]
...
print('\n')
print(
    f'there are {n_pairs} lines in {args.im_anns}, which means {n_pairs} image/label image pairs'
)
print('\n')

print(
    f'max and min image shapes by area are: {max_shape_area}, {min_shape_area}'
)
print(
    f'max and min image shapes by height are: {max_shape_height}, {min_shape_height}'
)
print(
    f'max and min image shapes by width are: {max_shape_width}, {min_shape_width}'
)
print('\n')
