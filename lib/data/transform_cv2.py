...
class RandomResizedCrop(object):
    ...
    def __call__(self, im_lb):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            lb = np.pad(lb, ((pad_h, pad_h), (pad_w, pad_w)),
                        'constant',
                        constant_values=255)
        ...
        return dict(im=im[sh:sh + crop_h, sw:sw + crop_w, :].copy(),
...
class ColorJitter(object):
    ...
    def adj_saturation(self, im, rate):
        M = np.float32([[1 + 2 * rate, 1 - rate, 1 - rate],
    ...
    def adj_brightness(self, im, rate):
        table = np.array([i * rate
                          for i in range(256)]).clip(0, 255).astype(np.uint8)
        return table[im]

    def adj_contrast(self, im, rate):
        table = np.array([74 + (i - 74) * rate
                          for i in range(256)]).clip(0, 255).astype(np.uint8)
        return table[im]
...
class ToTensor(object):
    ...
    def __call__(self, im_lb):
        ...
        mean = torch.as_tensor(self.mean, dtype=dtype, device=device)[:, None,
                                                                      None]
        std = torch.as_tensor(self.std, dtype=dtype, device=device)[:, None,
                                                                    None]
